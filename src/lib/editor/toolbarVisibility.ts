...
export class ToolbarVisibility {
  ...
  selectionChanged(empty: boolean): void {
    ...
    // Непустое выделение при уже видимой панели — оставляем как есть: панель
    // уже показана для текущего сеанса видимости, повторно её не прячем и не
    // переотсчитываем debounce (иначе был бы лишний цикл onChange false→true).
    if (this._visible) return;
    // Панель ещё скрыта: (пере)запускаем таймер. Так движение выделения
    // переотсчитывает 150мс заново и не показывает панель раньше стабилизации.
    this.cancelTimer();
