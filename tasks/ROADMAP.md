...
> Обновлено: 2026-06-07 (добавлены MDP-39, MDP-40 — follow-up после SENAR-ревью MDP-38).
...
| 7a  | ★ **MDP-39** Tighten Tauri fs ACL scope (2pt, SENAR follow-up) | C        | MDP-7         |
...
| 27  | ★ **MDP-40** Replace `.expect()` in `src-tauri/src/lib.rs` (1pt, SENAR f-up) | B        | —                       |
| 28  | ★ **MDP-30** Performance benchmarks                                          | C        | большая часть редактора |
| 29  | **MDP-32** Cross-platform smoke testing                                      | —        | всё выше                |
| 30  | **MDP-33** Release 0.1.0                                                     | —        | MDP-32                  |
...
- **SENAR follow-ups из MDP-38.** [MDP-39](https://linear.app/mpegeev/issue/MDP-39) (Tauri ACL `$HOME/**` слишком широк — поднято в пункте 16 high-чеклиста, нужно делать сразу после MDP-7 когда определится реально нужный scope) и [MDP-40](https://linear.app/mpegeev/issue/MDP-40) (`.expect()` в `src-tauri/src/lib.rs:17` — поднято в пункте 18 high-чеклиста, можно делать в любой момент M2..M7, обязательно до MDP-33 release). Обе — pre-existing нарушения, попавшие в обзор при high-уровне MDP-38.
