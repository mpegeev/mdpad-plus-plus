...
/// (намеренно: при будущей смене формата granted-корни сбрасываются, и
/// пользователь повторно выдаёт доступ через диалог — это лишь runtime-кэш,
/// не критичные данные).
...
///
/// Read-modify-write не синхронизирован между процессами: предполагается, что
/// гранты приходят из нативных диалогов, которые модальны и сериализованы
/// (одновременных pick_* нет). Этого достаточно для текущего сценария.
