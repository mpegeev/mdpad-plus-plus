...
**Для задач, где правильный ответ известен заранее** (например: парсер
markdown, проверка пути, конвертация формата, валидатор):

- Тесты пишет отдельный субагент `test-writer`.
- Ему дают только критерии приёмки и сигнатуры функций
  (типы аргументов и возвращаемого значения) — **без кода реализации**.
- Разработчик эти тесты не редактирует — даже если они падают.
- Если они падают — либо критерии сформулированы нечётко,
  либо в коде ошибка. И то, и другое надо разбирать.

Зачем это: разработчик-агент не может подогнать тесты под свой код,
потому что тесты пишет другой агент, который этот код не видит.
Это структурная защита, дополнение к SENAR-ревью. Подробнее — в
`.claude/agents/test-writer.md` и `CONTRIBUTING.md`.
