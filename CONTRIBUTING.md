...
1. Всегда читать [`CLAUDE.md`](./CLAUDE.md) перед началом работы.
2. Следовать методологии SENAR: [`docs/senar.md`](./docs/senar.md) —
   шлюзы "Старт" / "Готово", чеклист ревью, правила безопасности.
3. Брать ровно одну задачу из Linear за раз. Один PR = один issue.
4. Внимательно следовать acceptance criteria — не больше и не меньше.
5. Не выходить за область изменений задачи без согласования.
6. Обновлять `CLAUDE.md`, если задача меняет архитектуру или вводит конвенции.
7. После завершения — переводить issue в `In Review`, открывать PR с `Closes MDP-XXX`.

### Slash-команды Claude Code

Полный список — в [`CLAUDE.md`](./CLAUDE.md). Ключевые шлюзы:

- `/new-task <имя>` — шлюз **"Старт"**: формулировка цели, критериев,
  негативных сценариев, области и уровня риска. Создаёт `tasks/<имя>.md`.
- `/done <имя>` — шлюз **"Готово"**: сбор свидетельств по каждому
  критерию, фиксация тупиков и решений, SENAR-ревью.
- `/rca <описание>` — root cause analysis для найденного дефекта.
- `Use the senar-reviewer subagent` — ревью diff'а по чеклисту SENAR
  перед коммитом.
